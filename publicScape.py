...
try:
    # Prefer modern champion mapping via Data Dragon
    from providers.ddragon import load_champion_map
except Exception:
...
champ_map = {}
if load_champion_map:
    try:
        champ_map = load_champion_map(base_dir)
    except Exception:
        pass

if not champ_map:
    # Fallback to local champions.json in repo root
    local_path = base_dir / 'champions.json'
    with local_path.open('r', encoding='utf-8') as champJson:
        champArray = json.load(champJson)
        champ_map = champArray.get('data', {})
...
for champ in sChampFBCalc:
    name = champ_map.get(int(champ), {}).get('name', str(champ))
    print('{} fb rate is {}% Sample size: {}'.format(name, round(100 * ChampFBCalc[champ] ), str(ChampTotalPlayed[champ])))
